...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--balanced",
        action="store_true",
        help="use balanced train dataset (only for waterbirds)"
    )
    # logging
    ...
    parser.add_argument("--vanilla", action="store_true", help="whether to test vanilla")
    parser.add_argument("--out_csv", help="output csv file for test results", type=str, default="test_results.csv")
    
    # experiment
    parser.add_argument("--pretrained_path", help="path for pretrained model", type=str)
    parser.add_argument("--worst_group", action="store_true", help="whether to use worst-group accuracy for model selection")
    ...
    if args.vanilla:
        acc = learner.test_vanilla(args)
        with open(os.path.join(args.pretrained_path, "test_result.txt"), "w") as f:
            f.write(f"{acc:.4f}")
