...
class FairfaceDataset(ConfounderDataset):
    def __init__(
        self,
        root_dir,
        target_name,
        ...
        self.confounder_array = self.metadata_df["ethnicity"].values
        self.n_confounders = 1

        self.img_paths = [
            os.path.join(self.data_dir, fn)
            for fn in self.metadata_df["file"]
        ]
        ...
        self.train_transform = get_transform(self.model_type,
                                                    train=True,
                                                    augment_data=augment_data)
        self.eval_transform = get_transform(self.model_type,
                                                train=False,
                                                augment_data=augment_data)

def get_transform(model_type, train=False, augment_data=False):
