...
def get_spurious_col_csv(args):
    ...
    args.metadata_path = os.path.join(
        ...
        args.root_dir, args.metadata_csv_name
    )
    ...
    index_col = "Unnamed: 0"
    if args.dataset == "CUB" or args.dataset == "waterbirds":
    ...
    for method in methods:
        ...
        for up_weight in up_weights:
            ...
            with open(job_script_path, "w") as file:
                ...
                print(
                    f"python {args.cwd}/run_expt.py -s confounder -d {args.dataset} -t {args.target} -c {confounder_name}"
                    ...
                    + (" --save_best")
                    + (" --save_last")
                    + (f" --final_epoch {args.final_epoch}")
                )
            ...
            if args.deploy:
                subprocess.run(f"sh {job_script_path}", check=True, shell=True)
...
if __name__ == "__main__":
    ...
    if args.dataset == "CUB":
        args.root_dir = "./cub" if not args.root_dir else args.root_dir
        args.target = "waterbird_complete95"
        args.confounder_name = "forest2water2"
        args.model = "resnet50"
        args.batch_size = 64 if not args.batch_size else args.batch_size
        args.n_epochs = 100 if not args.n_epochs else args.n_epochs
        ...
        args.root_dir = "./waterbirds" if not args.root_dir else args.root_dir
        args.target = "y"
        args.confounder_name = "place"
        args.model = "resnet18"
        args.batch_size = 64 if not args.batch_size else args.batch_size
        args.n_epochs = 100 if not args.n_epochs else args.n_epochs
        args.memory = 30 if not args.memory else args.memory
        ...
        args.root_dir = "./fairface" if not args.root_dir else args.root_dir
        args.target = "gender"
        args.confounder_name = "ethnicity"
        args.model = "resnet18"
        args.batch_size = 64 if not args.batch_size else args.batch_size
        args.n_epochs = 100 if not args.n_epochs else args.n_epochs
        args.memory = 30 if not args.memory else args.memory
        ...
        args.root_dir = "./" if not args.root_dir else args.root_dir
        args.target = "Blond_Hair"
        args.confounder_name = "Male"
        args.n_epochs = 50 if not args.n_epochs else args.n_epochs
        args.model = "resnet50"
        args.memory = 30 if not args.memory else args.memory
        args.metadata_csv_name = "metadata.csv" if not args.metadata_csv_name else args.metadata_csv_name
    elif args.dataset == "MultiNLI":
...
#         args.lr = 0.00002
#         args.weight_decay = 0
        args.batch_size = 32 if not args.batch_size else args.batch_size
        args.model = "bert"
        args.memory = 30 if not args.memory else args.memory
        args.n_epochs = 5 if not args.n_epochs else args.n_epochs
        args.metadata_csv_name = "metadata.csv" if not args.metadata_csv_name else args.metadata_csv_name
    elif args.dataset == "jigsaw":
        args.root_dir = "./jigsaw" if not args.root_dir else args.root_dir
...
#         args.batch_size = 16 # no-bert-param: 24
        args.n_epochs = 3 if not args.n_epochs else args.n_epochs
