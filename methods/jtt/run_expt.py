...
def main(args):
    run_name = f"up_{args.up_weight}_fe_{args.final_epoch}_lr_{args.lr}_wd_{args.weight_decay}"

    wandb.init(
        project=f"{args.project_name}_{args.dataset}_{args.model}",
        name=run_name
    )
    ...
    if args.up_weight != 0:
        ...
        print("Upsampling points:", len(upsampled_points))
    ...
    if args.hinge:
        assert args.dataset in ["CelebA", "CUB", "Waterbirds", "fairface"]  # Only supports binary
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project_name", type=str, default="jtt", help="wandb project name")
    # Confounders
