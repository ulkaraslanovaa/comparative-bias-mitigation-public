...
def train(
    ...
    for epoch in range(epoch_offset, epoch_offset + args.n_epochs):
        ...
        # Comment out for hyperparameter tuning
        ...
        if args.save_best:
            if args.loss_type == "group_dro" or args.reweight_groups:
                ...
                logger.write(f"Current avg validation accuracy: {curr_val_acc}\n")
            ...
            if curr_val_acc > best_val_acc:
                ...
                val_result_path = os.path.join(args.log_dir, "val_result.txt")
                with open(val_result_path, "w") as vf:
                    vf.write(f"Epoch: {epoch}\tAccuracy: {curr_val_acc:.4f}\n")

            curr_worst_group_acc = val_loss_computer.worst_group_acc
            if curr_worst_group_acc > best_val_worst_acc:
                best_val_worst_acc = curr_worst_group_acc
                torch.save(model, os.path.join(args.log_dir, "best_worst_group_model.pth"))
                logger.write(f"Validation worst-group idx: {val_loss_computer.worst_group_idx}\n")
                logger.write(f"Current worst-group validation accuracy: {curr_worst_group_acc}\n")
                logger.write(f"Best worst-group model saved at epoch {epoch}\n")
                os.makedirs(args.log_dir, exist_ok=True)
                val_worst_path = os.path.join(args.log_dir, "val_worst_result.txt")
                with open(val_worst_path, "w") as wf:
                    wf.write(
                        f"Epoch: {epoch}\tWorstGroupIdx: {val_loss_computer.worst_group_idx}\tAccuracy: {curr_worst_group_acc:.4f}\n"
                    )
