...
def main(args):
    ...
    data_dir = f"{args.results_dir}/{args.dataset}/{exp_name}/{folder_name}/model_outputs/"

    if args.root_dir:
        metadata_path = args.root_dir + "/metadata.csv"
    else:
        if args.dataset == 'CelebA':
            ...
            metadata_path = "./waterbirds/data/metadata.csv"
        elif args.dataset == 'fairface':
            metadata_path = "./fairface/data/metadata.csv"
    ...
    if dataset == "CUB" or dataset == "waterbirds":
        merged_csv["spurious"] = merged_csv['y'] != merged_csv["place"]
    elif dataset == "fairface":
        merged_csv["spurious"] = merged_csv['gender'] != merged_csv["ethnicity"]
    elif dataset == "CelebA":
    ...
    if dataset == "MultiNLI":
        ...
        elif dataset == 'CUB' or dataset == 'waterbirds':
            train_probs_df["confidence"] = train_probs_df["y"] * train_probs_df["probs_1"] + (1 - train_probs_df["y"]) * train_probs_df["probs_0"]
        elif dataset == 'fairface':
            train_probs_df["confidence"] = train_probs_df["gender"] * train_probs_df["probs_1"] + (1 - train_probs_df["gender"]) * train_probs_df["probs_0"]
        elif dataset == 'jigsaw':
    ...
    if not os.path.exists(f"{args.results_dir}/{dataset}/{exp_name}/train_downstream_{folder_name}/final_epoch{final_epoch}"):
        os.makedirs(f"{args.results_dir}/{dataset}/{exp_name}/train_downstream_{folder_name}/final_epoch{final_epoch}")
    root = f"{args.results_dir}/{dataset}/{exp_name}/train_downstream_{folder_name}/final_epoch{final_epoch}"

    train_probs_df.to_csv(f"{root}/metadata_aug.csv")
    
    exp_name = f"{exp_name}/train_downstream_{folder_name}/final_epoch{final_epoch}"
    sbatch_command = (
        f"python {args.cwd}/generate_downstream.py --final_epoch {final_epoch} --exp_name {exp_name} --lr {args.lr} --weight_decay {args.weight_decay} --method JTT --dataset {args.dataset} --aug_col {args.aug_col} --cwd {args.cwd} --results_dir {args.results_dir} --deploy" + (f" --batch_size {args.batch_size}" if args.batch_size else "" ) + (f" --root_dir {args.root_dir}" if args.root_dir else "") + (f" --n_epochs {args.n_epochs}" if args.n_epochs else "")
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", type=str, default="CelebA", help="CUB, CelebA, MultiNLI, waterbirds, fairface"
    )
    parser.add_argument(
        "--final_epoch",
        type=int,
